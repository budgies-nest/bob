...
func main() {
	...
	mux.HandleFunc("POST /chat", func(response http.ResponseWriter, request *http.Request) {
		...
		if len(mcpTooCalls) > 0 {
			mcpResults, _ = workflow.ExecuteMCPToolCalls(response, flusher, khan, mcpTooCalls)
		}

		// STEP 3: TOOL CALLS EXECUTION
		if len(toolCalls) > 0 {
			workflow.ExecuteToolCalls(response, flusher, agentsCatalog, riker, selectedAgent, toolCalls)
		} else {
		...
		answer, errCompletion := selectedAgent.Agent.ChatCompletionStream(func(self *agents.Agent, content string, err error) error {
